...
// Build 组装卡片信息
func (Card) Build(repo, branch, author, email, status, commitMsg, repoUrl, droneUrl string,
...

	{
		var (
			tag     = "markdown"
			content = fmt.Sprintf("**分支**:  %s", branch)
		)
		card.Elements = append(card.Elements, &CardElement{
			Tag:     &tag,
			Content: &content,
...

	{
		var (
			tag     = "markdown"
			content = fmt.Sprintf("**提交者**:  %s [%s](mailto:%s)", author, email, email)
		)
		card.Elements = append(card.Elements, &CardElement{
			Tag:     &tag,
			Content: &content,
...
func handleMultipleLinesCommitMsg(commitMsg string) string {
	commitMsg = strings.Replace(commitMsg, "\\n", "\n", -1)
	...
	for _, line := range commitMsgLines {
		...
		markdownHolders = append(markdownHolders, line)
	}

	return strings.Join(markdownHolders, "<br>")
}
